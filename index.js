//Author&Thanksto jan Dihapus anjg
//by jere ganz
const {
    WAConnection,
   MessageType,
		...
		try {
          ...
          const checkLimit = (sender) => {
           ...
           return false
			...
			switch(command) {
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					if (!tex) return client.sendMessage(from, 'TEXTNYA MANA OM?', text, {quoted: mek})
               ...
               if (data.error) return reply(data.error)
                ...
                if (!isRegistered) return reply(ind.noregis())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (args.length < 1) return reply('Namamu apa om?')
               ...
               case 'help': 
         	   ...
         	   case 'creator':
                  ...
                  client.sendMessage(from, 'JERE GANZ NIH GENKK',MessageType.text, { quoted: mek} )
					break    
				case 'leaderboard':
				case 'lb':
                ...
                addKoinUser('6283191632340@s.whatsapp.net', fee)
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'toimg':
				if (!isRegistered) return reply(ind.noregis())
				if (!isQuotedSticker) return reply('Reply fotonya!')
					reply(ind.wait())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				try {
						if (args.length < 1) return client.sendMessage(from, 'namanya mana om?', text, {quoted: mek})
				...
				case 'add':
					...
					if (args.length < 1) return reply('Parah Main Nyuri')
           ...
           case 'demote':
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Ada Admin Baru Neh:\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`Ada Admin Baru Neh`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Mampus`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
